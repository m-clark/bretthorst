...
# plot p(delta|S,D_1,D_2,I)
 ...
 #grid()
 if(filling) polygon(c(delta.sek[1],delta.sek, delta.sek[length(delta.sek)]), c(0,pdelA,0), col=rgb(cols.rgb[1,1],cols.rgb[2,1],cols.rgb[3,1],0.5), border=NA)
...
#pdelwta
#MA: extract + sort according to delta.sek #pdelwta <- sort[RotateLeft[Take[sortrxd,{nÎ´i+1,nÎ´}],{0,1}]]
#tr <- t(pdelwta)
...
#prwta
#MA: extract + sort according to r.sek #prwta <- sort[RotateLeft[Take[sortrxr,{nÎ´i+1,nÎ´}],{0,1}]]
#tr <- t(prwta)
